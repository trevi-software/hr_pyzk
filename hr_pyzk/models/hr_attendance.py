...
class HrAttendance(models.Model):
    ...
    # I copied _check_validity() from odoo/addons/model/hr_attendance.py
    # I've modified it to return meaningful error instead of rasing exceptions.
    #
    @api.model
    def check_conflicting_attendance(self,
                                     employee_id,
                                     check_in,
                                     check_out=None):
        """ Verifies the validity of the attendance record compared to the
            others for the same employee. For the same employee we must have :
                * maximum 1 "open" attendance record (without check_out)
                * no overlapping time slices with previous employee records
            Returns a result containing two fields:
                "result" - string containing one of four values:
                    None                        - no conflicts were detected
                    "overlap_on_checkin"        - the check-in time straddles
                                                  another attendance
                    "multiple_open_attendances" - there is another "open"
                                                  attendance
                    "overlap_on_checkout"       - the check-out time straddles
                                                  another attendance
                "hr_attendance" - the 'hr.attendance' record conflicts with
                                  the proposed employee_id's check-in/
                                  check-out times
        """
